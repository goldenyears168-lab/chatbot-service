...
/**
 * 加载知识库数据（Edge Runtime 兼容）
 * @param company 公司 ID
 * @param baseUrl 基础 URL（用于访问 public 目录中的文件）
 * @returns 知识库数据
 */
export async function loadKnowledgeData(
  ...
  // 过滤掉 null 值并排序
  const validFiles = fileDataList.filter(f => f !== null) as KnowledgeFile[]
  
  validFiles.sort((a, b) => a.number - b.number)
  
  if (validFiles.length === 0) {
    throw new Error(`Knowledge directory not found for company: ${company}`)
  }
